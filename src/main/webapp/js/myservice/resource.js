var resource = {
    ...
    actionRoot: "/UCFCloudPortal",
	 init: function(url){
			...
			Request = getRequest();
			resource.serviceId = Request['serviceId'];    
			resource.resourcePoolsId = Request['resourcePoolsId'];
			resource.zoneId = Request['zoneId'];
...
function resUpdate(){
	getAllResourceList(); // 刷新列表，重新查询加载
    $("body").stopTime(timerKey);
    $("body").everyTime(50000,timerKey,function() {  
        query();
    });
}
...
//	resource.zoneId = Request['zoneId'];
	...
	$.ajax({
		type : "POST",
		url : "../cloud_mall/showResourceList.action?curPage="+$("#curPage").val()+"&pageSize="+pageSize,
		datatype : "json",
		data : param,
		async : false,
		global : false,
...
//	            	if(data.listResp.ResId == -1){
//	            		alert('此虚拟硬盘上次挂载，解挂虚拟机失败！');
//	            	}
				  ...
				  if(data.listResp.total != undefined) {
					...
					if (array != null && array.length > 0) { 
						$(array).each(function(i) {
							...
							$("#resource_tbody").append(tables(i, array[i], trEvents, data.listResp.initPassword));								
						});
					}else{
...
function tables(i, info, trEvents, initPassword) {
	
    var begintime_ms = Date.parse(new Date(info.createDt));
	var endtime_ms = Date.parse(new Date());
	var stepTime = endtime_ms-(begintime_ms+initPassword*24*60*60*1000);
	...
	if(1==templateType &&resource.isFirst ){
		if(info.password != null && stepTime < 0){
			resource.isFirst = false;
			var resNameAll = resName + '  初始密码：' + info.password;
			//alert('虚拟机的初始密码为' + info.password + '，为保证安全请及时修改密码。如果您已经修改了初始密码请忽略此信息。');
			$(".vm_password").html('云主机的初始密码为' + info.password + '，初始密码系统只保留'+initPassword+'天，请在'+initPassword+'天内修改初始密码。如果您已经修改了初始密码请忽略此信息。');
		} else {
	...
	// 删除了垃圾代码
	//var refResAll=ref;
	resName = resName.length >15 ? resName.substr(0,15) + '..' : resName;
	resourceInfo = resourceInfo !=undefined && resourceInfo.length > 25 ? resourceInfo.substring(0,25) + '..' : resourceInfo;
	templateTypeName = templateTypeName != undefined ? templateTypeName.replace('模板','') : '';
	comment = comment != undefined && comment.length > 21 ? comment.substr(0, 21) + ".." : comment;
	var vref='';
	if(ref!=undefined){
	...
	var dom = ''
          	+ '<tr '+trEvents+'>'
          	+ '	<td width=\'5%\' >'+resId+'</td>'
			+ '	<td width=\'10%\' title=\"'+resNameAll+'\"  style=\"text-align:center\">'+resName+'</td>'
			+ '	<td width=\'15%\' title=\"'+resall+'\" style=\"text-align:center\">'+resourceInfo+'</td>'// 资源配置明细
			+ '	<td width=\'10%\' style=\"text-align:center\">'+templateTypeName+'</td>'
			+ '	<td width=\'10%\' title=\"'+comall+'\" style=\"text-align:center\">'+comment+'</td>'
			+ '	<td width=\'10%\' id=\"ref_'+info.id+'\" title=\"'+refResAll+'\ "style=\"text-align:center\">'+vref+'</td>'
			+ '	<td width=\'8%\' id=\"state_'+info.id+'\">'+state+'</td>'// 资源状态显示
			+ '	<td width=\'8%\' class=\"shop last\" id=\"caozuo_'+info.id+'\">'
			+ operate(info)// 资源操作显示
			+ '	</td>'
			+ '</tr>';	
	return dom;
...
function operate(info){
    ...
    function getMonitorInfo(code){
		...
		info = info.replace("pm", "物理机");// 云监控服务，物理机未替换为中文
