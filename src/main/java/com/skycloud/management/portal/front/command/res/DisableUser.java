...
public class DisableUser extends BaseCommandPo{
	public static final String COMMAND = "disableUser";
	...
	protected QueryCommand fromJsonToOperatePo(String jsonStr) {
		try{
			if(StringUtils.isBlank(jsonStr)){
				throw new ServiceException("解析Json串失败[DisableUser]方法fromJsonToOperatePo：传入参数为空");
			}
			String tempJson = "";
			tempJson = fromJsonToMap(jsonStr).get(COMMAND).toString();
			DisableUser po = (DisableUser)JsonUtil.getObject4JsonString(tempJson, DisableUser.class);
			return po;
		}catch(Exception e){
			logger.error("解析Json串失败[DisableUser]方法fromJsonToOperatePo：",e);
			throw new RuntimeException("解析Json串失败[DisableUser]方法fromJsonToOperatePo："+e.getMessage());
		}
	}
